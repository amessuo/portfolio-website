import { Link, useLocation, useNavigate } from 'react-router-dom'
import { useSmoothScroll } from '../../hooks/useSmoothScroll'
import styles from './Footer.module.css'

export default function Footer() {
  const location = useLocation()
  const navigate = useNavigate()
  const scrollTo = useSmoothScroll()
  const isHome = location.pathname === '/'

  const handleNavClick = (id) => {
    if (isHome) {
      scrollTo(id)
    } else {
      navigate('/')
      setTimeout(() => scrollTo(id), 100)
    }
  }

  return (
    <footer className={styles.footer}>
      <div className={`${styles.inner} container`}>
        <div className={styles.brand}>
          <div className={styles.logo}>
            <span className={styles.logoGreen}>THREE</span>
            <span className={styles.logoWhite}>DIGITAL</span>
          </div>
          <p className={styles.tagline}>
            Strategy-led digital marketing that drives measurable growth for startups and growing businesses.
          </p>
          <div className={styles.socialIcons}>
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className={styles.socialIcon} aria-label="LinkedIn">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className={styles.socialIcon} aria-label="Twitter">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <a href="mailto:hello@threedigital.com" className={styles.socialIcon} aria-label="Email">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0L1.5 8.67z"/><path d="M22.5 6.908V6.75a3 3 0 00-3-3h-15a3 3 0 00-3 3v.158l9.714 5.978a1.5 1.5 0 001.572 0L22.5 6.908z"/></svg>
            </a>
          </div>
        </div>

        <div className={styles.col}>
          <h4 className={styles.colTitle}>Pages</h4>
          <button onClick={() => handleNavClick('about')} className={styles.footerLink}>About Us</button>
          <Link to="/case-studies" className={styles.footerLink}>Case Studies</Link>
          <button onClick={() => handleNavClick('contact')} className={styles.footerLink}>Contact Us</button>
        </div>

        <div className={styles.col}>
          <h4 className={styles.colTitle}>Our Services</h4>
          <span className={styles.footerLink}>SEO Booster</span>
          <span className={styles.footerLink}>Marketing Strategy</span>
          <span className={styles.footerLink}>Data Analysis</span>
          <span className={styles.footerLink}>The AI Marketing Build</span>
        </div>

        <div className={styles.col}>
          <h4 className={styles.colTitle}>Newsletter</h4>
          <p className={styles.newsletterText}>Get monthly insights on digital strategy and SEO.</p>
          <form className={styles.newsletterForm} onSubmit={(e) => e.preventDefault()}>
            <input type="email" placeholder="Your email" className={styles.emailInput} />
            <button type="submit" className={styles.submitBtn}>Subscribe</button>
          </form>
        </div>
      </div>

      <div className={`${styles.bottom} container`}>
        <p className={styles.copyright}>&copy; {new Date().getFullYear()} ThreeDigital. All rights reserved.</p>
        <div className={styles.bottomLinks}>
          <a href="#" className={styles.bottomLink}>Privacy Policy</a>
          <a href="#" className={styles.bottomLink}>Terms &amp; Conditions</a>
        </div>
      </div>
    </footer>
  )
}
